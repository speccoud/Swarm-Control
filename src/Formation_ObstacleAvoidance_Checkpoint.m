    ...
    for i=1:swarm_size
        ...
        for j=[1:(i-1),(i+1):swarm_size]
            ...
            if k >= 25
                %--- Destination Vector ---
                centroid = mean(swarm);
                % destination_vector = [checkpoints(checkpoint_index, 1) - swarm(i, 1), checkpoints(checkpoint_index, 2) - swarm(i, 2)];
                destination_vector = [checkpoints(checkpoint_index, 1) - centroid(1), checkpoints(checkpoint_index, 2) - centroid(2)];
        ...
        centroid = mean(swarm);
        dist_vector = norm([checkpoints(checkpoint_index, 1) - centroid(1), checkpoints(checkpoint_index, 2) - centroid(2)]);
        if dist_vector < 3 && checkpoint_index < length(checkpoints)
